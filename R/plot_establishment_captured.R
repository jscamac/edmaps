...
#'
#' @param data A `data.frame` as derived from [captured_by_ncells()].
#' @param xlab Character. x axis label.
#' @param ylab Character. y axis label.
#' @param legend_title Character. Optional legend title.
#' @param legend Legend position. Either a position name (one of
#'   `"none"`, `"top"`, `"bottom"`, `"left"`, or
#'   `"right"`, or a vector of two normalised coordinates ranging from 0
#'   to 1, e.g. `c(1, 0)` (for bottom-right).
...
#'  graphic device.
#' @param units Character. Units corresponding to `height` and
#'   `width`. Can be `"in"`, `"cm"`, or `"mm"`. Default is
#'   inches (`"in"`).
...
#' @return An image is written to `outfile` if provided, and otherwise a
