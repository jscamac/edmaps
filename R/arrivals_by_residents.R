#' Estimates pest arrivals by returning residents
#'
#' Estimates arrival rate attributable to returning residents as a function of
#' population density.
#'
#' @param pop_density A \code{RasterLayer} or file path to a raster file 
#'   containing population density.
#' @param total_returning Integer. The number of returning residents.
#' @param probability Numeric. The probability a returning resident carries pest
...
#' @export
arrivals_by_residents <- function(pop_density, total_returning, probability,
  ...
  out <- disperse_arrivals(prop_pop, total_returning, probability)
  names(out) <- c("arrivals_by_residents")
