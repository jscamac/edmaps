...
#' Estimates pest arrivals by vessels.
#'
#' @param port_weight A [`SpatRaster`] or `RasterLayer` object, or a file path
#'   to a raster file containing weights to be used to distribute arrivals.
...
arrivals_by_vessels <- function(port_weight, leakage_rate, establishment_rate,
  ...
  if(is.character(port_weight) || is(port_weight, 'RasterLayer')) {
    port_weight <- terra::rast(port_weight)
  } else if(!is(port_weight, 'SpatRaster')) {
    stop('port_weight must be a SpatRaster or RasterLayer object, ',
         'or a file path to a raster file.')
  }

  # Disperse passengers and calculate arrival rate
  EE <- calc_EE(leakage_rate, establishment_rate)
  out <- calc_pathway_pr(EE, port_weight, return_rast=TRUE)
