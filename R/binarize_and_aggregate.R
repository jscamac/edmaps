...
#' @param crs Coordinate reference system expressed in a form accepted by
#'   [terra::crs()] (e.g. `'ESRI:54009'`, WKT, or a `Spat*` object).
...
#' @importFrom terra cellFromXY crds crs ext project rast res vect writeRaster xyFromCell
#' @importFrom methods is
#' @export
binarize_and_aggregate <- function(infile, rle, outfile, extent, crs, res,
  categories, overwrite=FALSE, return_rast=FALSE, quiet=FALSE) {
  ...
  if(missing(crs)) crs <- attr(rle, 'crs')
  ...
  i <- which(rle$value %in% categories)
  r0 <- terra::rast(
    terra::ext(attr(rle, 'extent')), res=attr(rle, 'res'), crs=attr(rle, 'crs')
  )
  r1 <- terra::rast(terra::ext(extent), res=res, crs=crs, vals=0)
  # below conditional: if no values are members of the `categories` set, return early
  if(length(i) > 0) {
    x2 <- cumsum(rle$length[i])
    timespent <- Sys.time() - now
    if(!isTRUE(quiet)) {
    ...
    result <- unique(unlist(lapply(seq_along(x3), function(j) {
      ...
      z <- unlist(mapply(
        seq.int, rle$start[x3[[j]]], length.out=rle$length[x3[[j]]],
        SIMPLIFY=FALSE))
      xy0 <- terra::xyFromCell(r0, z)
      xy1 <- terra::vect(xy0, crs=attr(rle, 'crs'))
      if(crs != attr(rle, 'crs')) xy1 <- terra::project(xy1, crs)
      cell1 <- as.integer(setdiff(terra::cellFromXY(r1, terra::crds(xy1)), NA))
      cell1
    })))
    cat('\n')
    timespent <- Sys.time() - now
